  ...
  try {
    ...
    if (!isNaN(page) && !isNaN(limit) && page > 0 && limit > 0) {
      ...
      return res.json({
        success: true,
        users: listofUsers,
        message: `Showing page ${page} with ${limit} users per page`,
      });
    } else {
      const listofUsers = await Users.find().sort({ createdAt: -1 }); // Sort all users by createdAt field in descending order
  ...
  try {
    ...
    if (cvFile) {
      ...
      return res.json({
        success: true,
        userData: user,
        message: "CV updated Sucessfully.",
      });
    }
  } catch (error) {
    console.log(error), res.json(error);
